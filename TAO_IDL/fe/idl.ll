...
idl_parse_line_and_file (char *buf)
{
  ...
  *r++ = 0;
  long line_num = (long) idl_atoui (h, 10);
  ...
  for (; *r != '"'; r++)
    {
      if (*r == '\n' || *r == '\0')
        {
          // No filename, just set line number if valid (not 0)
          if (line_num != 0)
            {
              idl_global->set_lineno (line_num);
            }
          return;
        }
    }
  ...
  *r = 0;
  ...
  // Ignore modern g++ preprocessor special file names
  // These are generated by g++ -E and would cause errors
  if (ACE_OS::strcmp (h, "<command-line>") == 0 ||
      ACE_OS::strcmp (h, "<built-in>") == 0)
    {
      return;
    }

  // For # 0 directives from g++ preprocessor, set line number to 1 instead of 0
  // g++ generates "# 0 filename" directives which are invalid line numbers
  if (line_num == 0)
    {
      line_num = 1;
    }

  // Ignore temporary preprocessor files (they start with /tmp//idli_)
  // These are intermediate files created during preprocessing
  if (ACE_OS::strncmp (h, "/tmp//idli_", 11) == 0)
    {
      idl_global->set_lineno (line_num);
      return;
    }
